...
% save(strcat('S_K_',num2str(n)),'Sim_K')
% save(strcat('S_I_',num2str(n)),'Sim_I')
% save(strcat('S_PC_',num2str(n)),'Sim_PC')
% % ================================= %

%% Evaluate and plot
% Load Data
load(strcat('r_',num2str(n)))
load(strcat('S_I_',num2str(n)))
load(strcat('S_K_',num2str(n)))
load(strcat('S_PC_',num2str(n)))
...
%for I = 1:length(r.d)
%    I
%    scatter(log10(r.d(I)),log10(r.y(I)),5,log10(Ratio(I)),'filled','markerfacealpha',Ratio_PC_Scaled(I))
%end
scatter(log10(r.d),log10(r.y),5,log10(Ratio),'filled','markerfacealpha',0.6)
hold on
x = log10(r.d);
y =log10(r.y);
z = log10(Ratio);
...
cbh=colorbar;
%cbh.Ticks = [0:0.5:5];
xlabel('Diffusion Rate [L/hr]')
ylabel('Leakage [mmol/g]')
yticks([-2,-1,0,1])
xticks([-6,-5,-4,-3])
yticklabels({'10^{-2}','10^{-1}','10^{0}','10^{1}'})
xticklabels({'10^{-6}','10^{-5}','10^{-4}','10^{-3}'})
axis([r1 r2 y1 y2])
set(gca,'fontsize',14)
...
% leakage stoich (log10)
leakage = [0.1,-1,0.1];
% 0.1 pore size
%diffusion = [log10(2.39*10^-5),log10(2.39*10^-5),log10(2.39*10^-5)];
% diffiusion rates (log10)
diffusion = [-3.5,-4.5,-5.5];
...
for I = 1:length(diffusion)
    I
    figure(1)
    plot(diffusion(I),leakage(I),'o','markersize',7,'markeredgecolor',[0.5 0 0],'markerfacecolor',[1 0.5 0.5])
...
%     plot(diffusion(I),leakage(I),'r.','markersize',15)
    ...
    hold on
    plot(t,x3_1(:,7),'-','linewidth',lw3,'color',blue1)
    plot(t,x3_1(:,9),'-','linewidth',lw3,'color',green1)
    plot(t,x3_2(:,7)+x3_2(:,9),'linewidth',lw3,'color',c_pc,'linestyle',':')
    xlabel('Time [hrs]')
    ylabel('Biomass [g]')
    ...
    hold on
    plot(t,x3_1(:,8),'-','linewidth',lw3,'color',blue1)
    plot(t,x3_1(:,10),'-','linewidth',lw3,'color',green1)
    plot(t,x3_2(:,8)+x3_2(:,10),'linewidth',lw3,'color',c_pc,'linestyle',':')
    xlabel('Time [hrs]')
    set(gca,'fontsize',fntsz)
    set(gca,'yscale','log')
    axis([0 tspan(end) 0 6*10^-4])
    set(gca,'xtick',0:24:tspan(end))
    set(gca,'ytick',[10^-4.5,10^-4,10^-3.5])
    set(gca,'yticklabels',{'10^{-4.5}','10^{-4.0}','10^{-3.5}'})
    set(gca, 'YMinorTick','off')
...
figure(3)
set(gcf,'renderer','painters','position',[0 0 400 450])

%% ABC Distribution Plot (End Point Ratio Data)
figure(4)
% Plot histogram of all points
h = histogram(log10(r.y),'normalization','pdf','facealpha',0.5,'facecolor','k','Orientation','horizontal');
...
% figure(1)
% set(gcf,'renderer','painters','position',[0 0 500 400])
% saveas(gcf,'Ratio_2.svg')
% 
% figure(3)
% set(gcf,'renderer','painters','position',[0 0 400 600])
% saveas(gcf,'Examples_2.svg')
% 
% figure(4)
% set(gcf,'renderer','painters','position',[0 0 250 400])
% saveas(gcf,'Histogram_2.svg')
